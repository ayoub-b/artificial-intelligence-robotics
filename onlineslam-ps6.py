...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for p in range(len(data)):
        ...
        measurement = data[p][0]
        motion      = data[p][1]
    
        # integrate the measurements
        #print 'Previous Omega :'
        #Omega.show()
        ...
        for i in range(len(measurement)):
            ...
            for b in range(2):
                ...
                Xi.value[n+b][0]      += -measurement[i][1+b] / measurement_noise
                Xi.value[m+b][0]      +=  measurement[i][1+b] / measurement_noise
                #print 'Updating omega at :', m, b, measurement[i][0]
                #print Omega.value[m+b][m+b]

	    #expand the matrix
        indexes = [row for row in range(dim+2)]
        del indexes[2]
        del indexes[2]
        #print indexes
        #print 'Showing Omega :'
        #Omega.show()
        Omega = Omega.expand(dim+2, dim+2, indexes, indexes)
        Xi = Xi.expand(dim + 2, 1, indexes, [0])
        #print 'Expanded Omega :'
        #Omega.show()
        # update the information maxtrix/vector based on the robot motion
        for b in range(4):
            Omega.value[n+b][n+b] +=  1.0 / motion_noise
        for b in range(2):
            ...
            Xi.value[n+b+2][0]        +=  motion[b] / motion_noise
        # Update Omega and Xi
        indexesToTake = [row for row in range(2, dim+2)]
        #print indexesToTake
        smallOmega = Omega.take(indexesToTake, indexesToTake)
        smallXi = Xi.take(indexesToTake, [0])
        A = Omega.take([0,1],indexesToTake)
        ...
        #print 'Taken from:'
        #Omega.show()
        Omega = smallOmega - A.transpose() * B.inverse() * A
        Xi = smallXi - A.transpose() *B.inverse() * C
