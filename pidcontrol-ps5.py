...
############## ONLY ADD / MODIFY CODE BELOW THIS LINE ####################
    ...
    def cte(self, radius):
        ...
        y=self.y
        cte=0
        orientation=(self.orientation%(2*pi))/(2*pi)*360
        if x <= radius:
            d = sqrt((x - radius)**2.0 + (y-radius)**2.0) - radius
            cte=d
        elif x >= 3*radius:
            d = sqrt((x - 3.0*radius)**2.0 + (y-radius)**2.0) - radius
            cte=d
        elif y > radius:
            cte=y-2.0*radius
        elif orientation > 179.0 and orientation < 181.0 and x < 4*radius :
            cte=-y
        return cte
